    ...
    >
      ...
      >
        <ul class="flex flex-col items-center content">
          <li
            :ref="setLyricRefs"
            v-for="(lrc, index) in lyric"
            :class="{ 'text-emerald-400': index === currentLineNum }"
            :key="lrc.time"
          >
            {{ lrc.txt }}
          </li>
...
import { watch, ref, nextTick, watchEffect, toRaw } from "@vue/runtime-core";
...
watch(lrc, (val) => {
  if (Object.keys(lyricObj.value).length) {
    return;
  }
  ...
  l.seek(0);
  lyric.value = l.lines;
  playerStore.setLyric(l);
});
watch(isPlaying, (val) => {
  console.log("playing");
  if (val) {
    lyricObj.value.togglePlay(currentTime.value * 1000);
  } else {
    lyricObj.value.stop();
  }
});
